...
export function LineItem(LineItemProps) {
   ...
   return (
      <>
               ...
               onMouseLeave={modalPartner.onToggle} onMouseEnter={modalPartner.onToggle} >
                  <Image borderRadius={1} position='relative' h='100%' objectFit='cover'  objectPosition={'center'} src={LineItemProps.image} alt='Banner inicial'
                   />
                  
                  <Collapse in={modalPartner.isOpen} unmountOnExit={true}>
                  <Grid opacity={1} bg='#ffffff' w='100%' p={2} position='absolute' bottom='0' left='0' right='0' fontFamily={'Roboto'}>
                     <Text textAlign='center' color='#000000bb'fontSize='2rem'
                      fontWeight='300'>
                        {LineItemProps.text}
                     </Text>
                  </Grid>
